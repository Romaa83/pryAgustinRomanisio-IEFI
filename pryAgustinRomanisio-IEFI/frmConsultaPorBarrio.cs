...
{
    public partial class frmConsultaPorBarrio : Form
    {
        ...
        public frmConsultaPorBarrio()
        {
            InitializeComponent();
        }

        private void frmConsultaPorBarrio_Load(object sender, EventArgs e)
        {
        ...
        private void btnListar_Click(object sender, EventArgs e)
        {
            ...
            if (cboBarrio.SelectedIndex != -1) 
            {
                ...
                {
                    if (lector.GetString(1) == cboBarrio.Text) //Si es igual
                    {
                        ConexionBD2.Open(); 
                        ...
                        while (lector2.Read()) //Se recorre la base de datos SOCIOS 
                        {
                            ...
                            if (lector2.GetInt32(3) == lector.GetInt32(0)) //SI el numero de ID_Barrio es el mismo
                            {
                                ...
                                OleDbDataReader lectorActividad = ComandoBD3.ExecuteReader();

                                while (lectorActividad.Read() && lectorActividad.GetInt32(0) != lector2.GetInt32(4))
                                {
                                }
                                dgvListarBarrio.Rows.Add(lector2.GetInt32(0), lector2.GetString(1), lector2.GetString(2), //Se agrega a la grilla
                                    lector.GetString(1), lectorActividad.GetString(1), lector2.GetDecimal(5));
                                ConexionBD3.Close();
                            }
                        }
                    }
                }
                txtPromedio.Text = Convert.ToString(Total / contador);
            ...
            {
                MessageBox.Show("Ingrese un barrio");
            }
        }
